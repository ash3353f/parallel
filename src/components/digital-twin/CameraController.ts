...
import { DigitalTwinState, FacilityId, WorldMode } from "./digitalTwinTypes";
...
export class CameraController {
  ...
  public cameraTo(
    ...
    gsap.to(o, {
      ...
      onComplete: () => {
        this.controls.enabled = this.state.explore && this.state.mode === "WORLD";
        onDone?.();
      },
    });
  }
  ...
  public enterHQ() {
    ...
    const hqBuilding = this.bSystem.buildings.hq;
    const c = hqBuilding ? hqBuilding.center : ([-40, -25] as [number, number]);
    this.cameraTo(
  ...
  public enterFactory() {
    ...
    const facBuilding = this.bSystem.buildings.factory;
    const c = facBuilding ? facBuilding.center : ([0, -24] as [number, number]);
    this.cameraTo(
  ...
  public exitInterior() {
    const prevId = this.state.selectedId;
    this.exitInteriorSilent();
    this.state.mode = "WORLD";
    if (prevId) {
      this.focusBuilding(prevId);
    } else {
      this.worldOverview();
    }
  }
